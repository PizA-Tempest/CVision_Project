...
class TestCompareEducation(FeatureTwoTestCase):
    ...
    def test_ut_2_04_004_no_requirement_scores_neutral(self):
        # Was 1.0 — full marks for a listing that stated no qualification,
        # which let an unstated requirement outscore a satisfied one. Asserts
        # the constant rather than a literal so tuning it does not require
        # editing the test again.
        result = match_service.compare_education(CV_BSC_SE, None)
        self.assertEqual(result.ratio, match_service.UNSTATED_EDUCATION_RATIO)
        self.assertTrue(result.meets_requirement)
    ...
    # UT-2-04-007 removed: it called match_service._degree_level directly, a
    # private helper that is not in the Method Description, so it could not be
    # traced to a requirement. The behaviour it guarded — "Chiang Mai" not
    # being read as a Master's — is now enforced by _degree_level's own
    # word-boundary matching, and no input through M-02-04 distinguishes the
    # two implementations, so nothing testable was lost.
...
class TestCompareExperience(FeatureTwoTestCase):
    ...
    def test_ut_2_05_006_no_requirement_with_experience(self):
        # Was 1.0; see UT-2-04-004. UT-2-05-007 below is a different branch —
        # no requirement *and* no experience — and is unchanged at 0.5.
        self.assertEqual(match_service.compare_experience(THREE_YEARS, None).ratio,
                         match_service.UNSTATED_EXPERIENCE_RATIO)
...
# ---------------------------------------------------------------------
# UT-2-24 / UT-2-25 — CV ownership (not a documented method; Feature 5 groundwork)
# ---------------------------------------------------------------------

class TestCVOwnership(FeatureTwoTestCase):

    def test_ut_2_24_001_ownership_is_recorded(self):
        write_cv_registry(self.registry, [cv_record()])
    ...
    def test_ut_2_24_003_database_failure_is_survivable(self):
